 ...
 */
export const formatPriceAlertNotification = (ticker: string, currentPrice: number, alertPrice: number, alertType: string) => {
 ...
 */
export const formatNewsNotification = (ticker: string, newsCount: number, sentiment: number) => {
 ...
 */
export const formatPortfolioNotification = (type: 'success' | 'error' | 'warning' | 'info', message: string, data: Record<string, any> = {}) => {
  const priority: 'high' | 'medium' = type === 'error' ? 'high' : 'medium';
  return {
    type,

...
const SentimentShockCard = ({
  ...
  sentimentZScore: any;
  zScoreInterpretation: any;
  zScoreHistoricalMean: any;
  zScoreHistoricalStd: any;
  zScoreDaysOfHistory?: number;
  zScoreQuality: any;
  currentScore: any;
  className?: string;
  context?: string;
  ...
  const generateExplanation = () => {
    ...
    const diffPercent = Math.abs((diff / Math.abs(zScoreHistoricalMean)) * 100).toFixed(0);
    const direction = diff > 0 ? 'higher' : 'lower';

    return `Current sentiment is ${diffPercent}% ${direction} than ${zScoreDaysOfHistory}-day average`;
  };

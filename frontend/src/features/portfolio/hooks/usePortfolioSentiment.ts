    ...
    const fetchPortfolioSentiment = async () => {
      try {
        ...
        Object.entries(dailyData).forEach(([date, dayData]: [string, any]) => {
          (dayData.headlines || []).forEach((headline: any) => {
        ...
        let holdingsSentimentData = Array.from(holdingsMap.values()).map(holding => {
          ...
          holding.recentScores.sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        const aggregatedMetadata = {
          ...
          topSources: metadata.top_sources || Object.entries(metadata.source_breakdown || {})
            .map(([source, percentage]: [string, any]) => ({ source, percentage }))

...
export const useSectorData = (ticker: string | null, timeframe = '1Y', sector: SectorInput | null = null) => {
  ...
  const [data, setData] = useState({
    ...
    sentiment: {
      ...
      source_concentration_hhi: null,
      concentration_interpretation: null,
      top_sources: null,
      // Topic metrics
      dominant_topic: null,
      dominant_topic_weight: null,
      dominant_topic_percentage: null,
      topic_count: null,
      sentiment_by_topic: null,
      topic_weights: null,
      // Article count
    ...
    Promise.allSettled([fetchNews, fetchConstituents, fetchDailySentiment])
      .then(([newsRes, constRes, dailySentimentRes]) => {
        ...
        if (newsRes.status === 'fulfilled' && newsRes.value) {
          ...
          if (isAggregatedNews) {
            ...
            newData.sentiment = {
              ...
              volatility: null,
              sentiment_volatility: n.sentiment_volatility ?? null,
              volatility_quality: n.volatility_quality ?? null,
              breadth_score: null,
              num_bullish_mentions: null,
              num_bearish_mentions: null,
              breadth_interpretation: null,
              ...
              source_concentration_hhi: n.source_concentration_hhi ?? null,
              concentration_interpretation: n.concentration_interpretation ?? null,
              top_sources: n.top_sources ?? null,
              dominant_topic: n.dominant_topic ?? null,
              dominant_topic_weight: n.dominant_topic_weight ?? null,
              dominant_topic_percentage: n.dominant_topic_percentage ?? null,
              topic_count: n.topic_count ?? null,
              sentiment_by_topic: n.sentiment_by_topic ?? null,
              topic_weights: n.topic_weights ?? null,
              total_articles_analyzed: n.total_articles_analyzed ?? null
            };
          }
        } else {
          console.error('News fetch failed', (newsRes as any).reason || (newsRes as any).value);
        ...
        if (constRes.status === 'fulfilled' && constRes.value && constRes.value.success) {
          ...
          console.error('Top constituent fetch failed', (constRes as any).reason || (constRes as any).value);
        }
        ...
        if (dailySentimentRes.status === 'fulfilled' && dailySentimentRes.value) {
          ...
          console.error('Daily sentiment fetch failed', (dailySentimentRes as any).reason || (dailySentimentRes as any).value);
        }
